...
def print_answer(query: str, debug: bool = False, render_markdown: bool = False) -> str:
    """
    render_markdown=True → 用 Jupyter Markdown 渲染（链接可点击）
    render_markdown=False → 文本打印
    返回：格式化后的答案字符串
    """
    # 清空 retriever 内部的 logs，避免累积
    ...
    if selected_tables:
        for i, t in enumerate(selected_tables, 1):
            if render_markdown:
                output_lines.append(_mk_table_item(i, t["caption"], t["url"]))
            else:
                output_lines.append(f"Table{i}: {t['caption']}，url：{t['url']}")
    
    if selected_figs:
        for i, f in enumerate(selected_figs, 1):
            if render_markdown:
                output_lines.append(_mk_figure_item(i, f["caption"], f["url"]))
            else:
                output_lines.append(f"Fig.{i}: {f['caption']}，url：{f['url']}")
    ...
    if used_sources:
        if render_markdown:
            ...
            for i, s in enumerate(used_sources, 1):
                ...
                doi = (meta.get("doi") or "").strip()
                tail = f", DOI: {doi}" if doi else ""
                output_lines.append(f"[{i}] {title}{tail}")
    ...
    if debug:
        ...
        for lg in retr_logs:
            ...
            debug_lines.append(f"[DBG][{stage}] {json.dumps(payload, ensure_ascii=False)}")

        if all_items:
            def _dbg_sort_key(it):
                return (it['kind'], _num_key(it['raw_no']))
            all_items_sorted = sorted(all_items, key=_dbg_sort_key)
            debug_lines.append("[DBG] Available charts in context (sorted by number):")
            for it in all_items_sorted:
                debug_lines.append(f"  - ({it['kind']}) src={it['source']} raw={it['raw_no']} cap={_short(it['caption'])} url={it['url']}")

        debug_lines.append("[DBG] LLM <CHARTS> used (ordered):")
        debug_lines.append("  tables:" + json.dumps(cites.get("tables"), ensure_ascii=False))
        debug_lines.append("  figures:" + json.dumps(cites.get("figures"), ensure_ascii=False))

        if used_sources:
            debug_lines.append("[DBG] LLM <SOURCES> used (after filtering, ordered):" + json.dumps(used_sources, ensure_ascii=False))
        else:
            debug_lines.append("[DBG] LLM <SOURCES> is empty after filtering (no sources cited).")

        debug_lines.append("[DBG] ------- END DEBUG -------")
...
class PrintAnswerInput(BaseModel):
    query: str = Field(description="The query to ask about alloy oxidation.")
    debug: bool = Field(default=False, description="Whether to print debug logs.")
    render_markdown: bool = Field(default=False, description="Whether to render the output in Markdown.")
...
@tool(return_direct=True, args_schema=PrintAnswerInput)
def print_answer_tool(query: str, debug: bool = False, render_markdown: bool = False) -> str:
    ...
    render_markdown = _to_bool(render_markdown)

    # 直接拿返回字符串（你已将 print_answer 改为返回字符串）
    text = print_answer(query, debug=debug, render_markdown=render_markdown)

    # 若需要可点击渲染，且在 IPython/Notebook 环境，进行一次显示
    if render_markdown:
        try:
            from IPython.display import display, Markdown  # 仅在 Notebook 下可用
            display(Markdown(text))
        except Exception:
            # 非 Notebook 场景或前端不支持渲染时，忽略渲染，照常返回字符串
            pass

    # 无论如何，工具都返回字符串给 Agent（便于日志/上下游处理）
    return text
