...
def create_handoff_tool(
    *,
    ...
    def handoff_to_agent(
        # ✅ 新增：由上游 LLM 填写的下一跳任务描述
        task_discription: Annotated[str, "The task description for the next agent."],
        # 原有参数
        ...
        # ✅ 将任务描述注入为一条用户消息，传给下一跳 agent
        task_message = {"role": "user", "content": task_discription}
        ...
        if len(last_ai_message.tool_calls) > 1:
            ...
            return Command(
                ...
                goto=[Send(agent_name, {**state, "messages": handoff_messages + [task_message]})],
            )
        # Handle single handoff
        else:
            if add_handoff_messages:
                handoff_messages = state["messages"] + [tool_message, task_message]
            else:
                handoff_messages = state["messages"][:-1] + [task_message]
            return Command(
                goto=agent_name,
                graph=Command.PARENT,
