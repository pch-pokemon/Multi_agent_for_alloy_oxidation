...
from typing import Any, Callable, Literal, Optional, Sequence, Type, Union, cast, get_args, Mapping # 新增导入
...
MODELS_NO_PARALLEL_TOOL_CALLS = {"o3-mini", "o3", "o4-mini", "glm-4-plus", "glm-4.5"}

#--------------------------
# 新增函数，为每一个智能体设置给自的 output mode
#--------------------------
def _resolve_output_mode(
    agent_name: str,
    output_mode: Union[OutputMode, Mapping[str, OutputMode], Callable[[str], OutputMode]],
) -> OutputMode:
    if isinstance(output_mode, str):
        # 全局设置
        return output_mode
    if callable(output_mode):
        # 按名字动态决定
        mode = output_mode(agent_name)
        if mode not in get_args(OutputMode):
            raise ValueError(f"Invalid output mode returned for agent '{agent_name}': {mode}")
        return mode
    # 映射：按名字覆盖，默认 last_message
    mode = output_mode.get(agent_name, "last_message")
    if mode not in get_args(OutputMode):
        raise ValueError(f"Invalid output mode for agent '{agent_name}': {mode}")
    return mode
#--------------------------
...
#-----------------------------
    ...
    # 修改 out_mode 允许 str / dict / callable
    output_mode: Union[
        OutputMode,
        Mapping[str, OutputMode],
        Callable[[str], OutputMode],
    ] = "last_message",
#-----------------------------
    ...
    **deprecated_kwargs: Unpack[DeprecatedKwargs],
    ...
    for agent in agents:
        ...
        # 这里按 agent.name 解析出专属的 output_mode
        agent_output_mode = _resolve_output_mode(agent.name, output_mode)
