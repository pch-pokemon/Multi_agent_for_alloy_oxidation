...
"""Mode for adding agent outputs to the message history in the multi-agent workflow
   在多智能体工作流中将智能体输出添加到消息历史记录的模式

- `full_history`: add the entire agent message history
  `full_history`：添加整个代理消息历史记录

- `last_message`: add only the last message
  `last_message`：仅添加最后一条消息
"""

MODELS_NO_PARALLEL_TOOL_CALLS = {"o3-mini", "o3", "o4-mini,","glm-4-flash", "glm-4-plus", "glm-4.5", "glm-4.5-air", "glm-4.5-flash"} # 不支持同步调用的模型名称
...
def _prepare_tool_node(
    ...
    agent_names: set[str],
    handoff_descriptions: dict[str, str] | None = None, # 增加自定义转接 描述词
) -> ToolNode:
    ...
    if handoff_destinations:
        ...
        handoff_tools = [
            create_handoff_tool(
                ...
                description=(handoff_descriptions or {}).get(agent_name), # 为每一个 agent 设置他们各自的转接描述词, 可以让llm快速分配决定让哪一个agent来完成任务。

                #workflow = create_supervisor(
                #    [rag_agent, ml_agent, thermo_agent, ref_agent],
                #    model=llm,
                #    handoff_descriptions={
                #        "RAG_agent":    "Retrieve superalloy oxidation mechanisms & context; preserves inline citations; use for explanations, not numbers.",
                #        "ML_agent":     "Predict oxidation mass gain (mg/cm²) given alloy & conditions (T °C, t h). Returns ranked comparisons & concise summary.",
                #        "Thermo_agent": "Query thermodynamic data (ΔG, pO₂, K) vs T(°C); supports exact, interpolation/extrapolation, and curves.",
                #        "Ref_agent":    "Return literature metadata (title, DOI/link) for given topic; no rewriting; format is reference list.",
                #    },
                #)
...
def create_supervisor(
    ...
    *,
    ...
    agent_output_modes: dict[str, OutputMode] | None = None, # 增加 Agent 的 output mode 自定义
    add_handoff_messages: bool = True,
    handoff_tool_prefix: Optional[str] = None,
    add_handoff_back_messages: Optional[bool] = None,
    supervisor_name: str = "supervisor",
    include_agent_name: AgentNameMode | None = None,
    handoff_descriptions: dict[str, str] | None = None, # 增加按 Agent 的功能描述映射的转接
) -> StateGraph:
    ...
    tool_node = _prepare_tool_node(
        ...
        agent_names,
        handoff_descriptions=handoff_descriptions, # 增加Agent转接描述
    )
    ...
    for agent in agents:
        per_mode = (agent_output_modes or {}).get(agent.name, output_mode)  # 增加可以自定义对不同智能体的 output mode 方法

        #agent_output_modes={
        #"RAG_agent": "last_message",    # 只要最终答复
        #"ML_agent":  "full_history",    # 需要看中间工具细节时
